...
public static class ExpansionConverter
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var transform in manifest.Transforms.Where(x => x.Mode.Equals("adapt_expansion_package", StringComparison.OrdinalIgnoreCase)))
        {
            cancellationToken.ThrowIfCancellationRequested();
            string xboxRelative = GetRequiredString(transform, "xbox_source");
            string xboxPath = SafeCombine(xboxRoot, xboxRelative);
            string outputPath = SafeCombine(stagingRoot, transform.Output);
            Directory.CreateDirectory(Path.GetDirectoryName(outputPath)!);

            byte[] output = ExpansionPackagePatcher.Build(xboxPath);
